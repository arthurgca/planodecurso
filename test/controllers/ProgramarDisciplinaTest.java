...
public class ProgramarDisciplinaTest extends TestBase {
    ...
    public void sucesso() {
        Result result = callAction(
            controllers.routes.ref.Planos.programar(p1.getId(), 4L, 2),
            sessaoAutenticada());
    ...
    public void erroMaximoDeCreditosExcedido() {
        Result result = callAction(
            controllers.routes.ref.Planos.programar(p1.getId(), 4L, 1),
            sessaoAutenticada());
    ...
    public void erroRequisitosInsatisfeitos() {
        Result result = callAction(
            controllers.routes.ref.Planos.programar(p1.getId(), 6L, 2),
            sessaoAutenticada());
        assertThat(status(result)).isEqualTo(BAD_REQUEST);
        assertThat(contentType(result)).isEqualTo("application/json");
        assertThat(charset(result)).isEqualTo("utf-8");
